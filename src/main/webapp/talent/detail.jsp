      ...
      <div id="tabs" style="background-color: #fff; border: none;">
		...
		<div id="tabs-3">
		  <aside class="side-sticky">
		    ...
		    <!----------------------------- 리뷰 작성 폼 start ----------------------------------->
		    <c:if test="${sessionScope.id != null && (sessionScope.sid != detail_vo.u_s_id)}">
		    ...
		    <c:forEach var="rvo" items="${review_vo}">
		      ...
		      <div class="re-card">
		        ...
		        <div class="review-footer">
		         <c:if test="${sessionScope.id eq rvo.uvo.u_id }">
		          ...
		          <c:if test="${sessionScope.sid eq detail_vo.u_s_id}">
		            <button type="button" class="review-btn ansBtn" data-nick="${rvo.uvo.u_nickname}">답변</button>
		          </c:if>
		        </div>
		        ...
		        <div class="re-reply" id="re${rvo.uvo.u_nickname}">
