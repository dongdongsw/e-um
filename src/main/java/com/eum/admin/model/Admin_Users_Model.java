...
public class Admin_Users_Model {
	...
	public String admin_users_list(HttpServletRequest request, HttpServletResponse response) {
		...
		map.put("start", start);
		map.put("end", end);
		List<UsersVO> users_list = Admin_UsersDAO.adminUsersListData(map);
		int totalpage = Admin_UsersDAO.adminUsersListTotalData();
		final int BLOCK = 10;
	...
	public String admin_users_detail(HttpServletRequest request, HttpServletResponse response) {
		...
		UsersVO users_vo = Admin_UsersDAO.adminUsersDetailData(u_id);
