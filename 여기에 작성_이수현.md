...
- test_(function) Branch
  ...
  - test 브랜치를 기반으로 생성되었으며, 각 기능별로 독립적으로 개발하여 충돌 없이 병렬 개발이 가능합니다.
  - 1차 개발이 완료되면 default 브랜치로 병합됩니다.

- default_(name) Branch
  - test 브랜치에서 분리된 팀원별 마무리·수정용 브랜치입니다.
  - default 브랜치에 1차적으로 개발된 기능을 통합한 이후, 개선 및 수정을 거쳐 default 브랜치로 병합됩니다.
...
### 브랜치 운영 방식
 ...
 2. test_{function} Branch 분리
      - test Branch를 기준으로 기능별로 세분화된 test_{function} Branch를 생성하여 병렬 개발 진행
 3. default Branch 생성 및 중간 통합
      - test 단계에서 개발된 기능을 통합하기 위해 default 브랜치 생성
      - 각 test_{functoin}
 4. 최종 병합
      - 모든 기능이 test Branch에 정상적으로 통합한 후
<br/>
<br/>
